<script>
	...
	export let radius
	export let latitudePOI
	export let longitudePOI
	...
	const addMapMarkers = (
		...
		validRows.forEach((row, index) => {
			...
			if (
				...
				let lookup_element =
					customIconOptions.find(
						(element) => element.label == key_field
					) ?? false
				...
				if (!lookup_element) {
					...
					mapMarkerOptions.icon = L.divIcon({
						html: lookup_element.value,
						className: "dummy",
					})
			...
			let latitudeFloat = parseFloat(latitudePOI)
			let longitudeFloat = parseFloat(longitudePOI)
			console.log(latitudePOI, longitudePOI, radiusInt)
			if (radiusInt && latitudeFloat && longitudeFloat) {
				L.circle([latitudeFloat, longitudeFloat], {
					radius: radiusInt * 1000,
					fillOpacity: 0.5,
				}).addTo(mapInstance)
			}

			let marker = L.marker(markerCoords, mapMarkerOptions).addTo(
				mapInstance
			)
			mapMarkerOptions = {
				icon: L.divIcon(defaultMarkerOptions),
